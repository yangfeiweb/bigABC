      ...
      <div class="choose-content">
        ...
        <div class="content-container">
          <transition name='fade'>
            <!-- 图片列表 -->
            <ul class="img-list" v-for="(item, idx) in wordList" :key="idx" v-if="idx==curr">
              <li class="item-container" v-for="(option,index) in item.options" :key="option.id" @click="isGoOn && check(option.word,index)">
                <div class='img-list-item'>
                  <bg-img cls='words-img' :src="option.imgUrl" alt=""></bg-img>
  ...
  methods:{
    ...
    createStage: function() {
      ...
      this.monster = new Sprite({
        texture: resources["monster"].texture,
        x: 0,
        y: 30
      });
      ...
      this.wingAnimate = new PIXI.extras.AnimatedSprite(Imgs1);
      this.wingAnimate.loop = true;
      this.wingAnimate.x = 0;
      this.wingAnimate.y = -20;
      this.wingAnimate.scale.set(0.7,0.7);
      this.wingAnimate.animationSpeed = 0.1;
      this.wingAnimate.play();  

      this.monsterContainer.addChild(this.bloodBg,this.blood,this.wingAnimate,this.monster);
      ...
      this.boonAnimate.scale.set(0.4,0.4);
      ...
      this.monsterAnimate.scale.set(0.8,0.8);
      ...
      this.dragnContainer.x=550;
      this.dragnContainer.y=80;
      let Imgs = [];
      this.dragnImgs.forEach(item => {
      ...
      this.dragnAnimate.scale.set(0.7,0.7);
        ...
        if(this.running){
          ...
          if(this.hitTest(this.star,this.monsterContainer)){
            ...
            if(this.blood.width<1){
              ...
              this.wingAnimate.visible=false;
    ...
    pause:function(){
      this.wingAnimate&&this.wingAnimate.stop();
      this.monsterMoving&&cancelAnimationFrame(this.monsterMoving);
      this.starRotating&&cancelAnimationFrame(this.starRotating);
    },
    playAgin:function(){
    ...
    check: function(word, index) {
      //防止重复点击
      if (this.chooseIdx !== -1) {
        return;
      }
      // 当所点击内容的word正确时 执行以下操作
      this.chooseIdx = index;
      if (this.currWord.word == word) {
        //当所点击的下标小于单词数量时
